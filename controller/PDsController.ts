...
export const generatedPDSExcel = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    getUserDataById(employee_id, async (err: Error, result: any) => {
      ...
      const sheet = workbook.getWorksheet("C1")
        ...
        if (children && children.length > 0) {
          children.forEach((child: any, index: number) => {
            ...
            if (child.childDateOfBirth) {
              ...
              sheet.getCell(`M${rowIndex}`).value = dob.toLocaleDateString(
                "en-PH",
                {
                  month: "2-digit",
                  day: "2-digit",
      ...
      try {
        ...
        education.forEach((educ: any, index: number) => {
          ...
          sheet.getCell(`N${rowIndex}`).value =
            educ.academicOrScholarshipReceived
        })
      } catch (err) {
      ...
      const civilStatusText =
        (result.civil_status.toLowerCase() === "single"
          ? "                  ✅"
          : "               ☐") +
        " Single " +
        (result.civil_status.toLowerCase() === "married"
          ? "                  ✅"
          : "               ☐") +
        " Married\n" +
        (result.civil_status.toLowerCase() === "widowed"
          ? "                  ✅"
          : "               ☐") +
        " Widowed " +
        (result.civil_status.toLowerCase() === "separated"
          ? "                  ✅"
          : "               ☐") +
        " Separated"
      ...
      const othersText =
        (result.civil_status.toLowerCase() === "others"
          ? "                  ✅"
          : "☐") + " Others"
      ...
      try {
        ...
        workExperience.forEach((work: any, index: number) => {
          ...
          if (work.inclusiveDateFrom) {
            ...
            sheetC2.getCell(`A${rowIndex}`).value = dateFrom.toLocaleDateString(
              "en-PH",
              {
                month: "2-digit",
                day: "2-digit",
          ...
          if (work.inclusiveDateTo) {
            ...
            sheetC2.getCell(`C${rowIndex}`).value = dateTo.toLocaleDateString(
              "en-PH",
              {
                month: "2-digit",
                day: "2-digit",
          ...
          const governmentServiceValue = Number(work.governmentService === 1)
            ? "Y"
            : "N"
      ...
      try {
        ...
        voluntaryWork.forEach((voluntary: any, index: number) => {
          ...
          if (voluntary.inclusiveDateFrom) {
            ...
            sheetC3.getCell(`E${rowIndex}`).value = dateFrom.toLocaleDateString(
              "en-PH",
              {
                month: "2-digit",
                day: "2-digit",
          ...
          if (voluntary.inclusiveDateTo) {
            ...
            sheetC3.getCell(`F${rowIndex}`).value = dateTo.toLocaleDateString(
              "en-PH",
              {
                month: "2-digit",
                day: "2-digit",
      ...
      try {
        ...
        trainingProgram.forEach((training: any, index: number) => {
          ...
          if (training.periodDateFrom) {
            ...
            sheetC3.getCell(`E${rowIndex}`).value =
              periodFrom.toLocaleDateString("en-PH", {
                month: "2-digit",
                day: "2-digit",
                year: "numeric",
              })
          ...
          if (training.periodDateTo) {
            ...
            sheetC3.getCell(`F${rowIndex}`).value = periodTo.toLocaleDateString(
              "en-PH",
              {
                month: "2-digit",
                day: "2-digit",
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=${result.first_name}-${result.surname}-PDS.xlsx`
      )
      res.setHeader(
        "Content-Type",
