...
export class batman1732036638759 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query("CREATE TABLE `users` (`user_id` int NOT NULL AUTO_INCREMENT, `username` varchar(50) NOT NULL UNIQUE, `password` varchar(255) NOT NULL, `role` enum('staff', 'admin', 'super_admin') NOT NULL DEFAULT 'staff', PRIMARY KEY (`user_id`)) ENGINE=InnoDB");

        await queryRunner.query("CREATE TABLE `super_admins` (`id` int NOT NULL AUTO_INCREMENT, `username` varchar(50) NOT NULL UNIQUE, `password` varchar(255) NOT NULL, `role` enum('staff', 'admin', 'super_admin') NOT NULL DEFAULT 'super_admin', `createdAt` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, `updatedAt` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, `isActive` boolean NOT NULL DEFAULT true, PRIMARY KEY (`id`)) ENGINE=InnoDB");

        await queryRunner.query("ALTER TABLE `employees_table` DROP COLUMN `username`, DROP COLUMN `password`, ADD COLUMN `user_id` int NULL, ADD CONSTRAINT `FK_employees_users` FOREIGN KEY (`user_id`) REFERENCES `users`(`user_id`) ON DELETE SET NULL ON UPDATE CASCADE");
        ...
        await queryRunner.query("CREATE TABLE `positions` (`id` int NOT NULL AUTO_INCREMENT, `position_name` varchar(100) NOT NULL, `categoryId` int NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB");
        
        await queryRunner.query("ALTER TABLE `positions` ADD CONSTRAINT `FK_positions_category` FOREIGN KEY (`categoryId`) REFERENCES `positions_category`(`id`) ON DELETE SET NULL ON UPDATE CASCADE");
        
        await queryRunner.query("ALTER TABLE `employees_table` ADD COLUMN `positionId` int NULL");
        
        await queryRunner.query("ALTER TABLE `employees_table` ADD CONSTRAINT `FK_employees_positions` FOREIGN KEY (`positionId`) REFERENCES `positions`(`id`) ON DELETE SET NULL ON UPDATE CASCADE");
